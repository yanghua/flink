...
public class QueryableStateProxyServiceHandler extends AbstractServerHandler<KvStateLocationRequest, KvStateLocationResponse> {

	private static final Logger LOG = LoggerFactory.getLogger(QueryableStateProxyServiceHandler.class);
	...
	private final QueryableStateProxyServiceImpl proxy;

	public QueryableStateProxyServiceHandler(
		final QueryableStateProxyServiceImpl proxy,
...
//		String registrationName = request.getQueryName();
//
//		KvStateLocation location = null;
//
//		//fetch it from cache
//		if (kvStateLocations.containsKey(jobId)) {
//			Map<String, KvStateLocation> kvStateLocationPerJob = kvStateLocations.get(jobId);
//			if (kvStateLocationPerJob.containsKey(registrationName)) {
//				location = kvStateLocationPerJob.get(registrationName);
//			}
//		}
//
//		if (location == null) {
//			CompletableFuture<KvStateLocation> locationFuture = proxy.getDispatcherGateway().requestKvStateLocation(jobId, registrationName);
//			try {
//				location = locationFuture.get(RPC_TIMEOUT.getSize(), RPC_TIMEOUT.getUnit());
//			} catch (TimeoutException e) {
//				LOG.error("Request kv state location timeout ({} {}) from dispatcher gateway.", RPC_TIMEOUT.getSize(), RPC_TIMEOUT.getUnit());
//			} catch (Exception e) {
//				LOG.error("Request kv state location failed from dispatcher gateway: ", e);
//			}
//		}
//
//		if (location != null) {
//			if (!kvStateLocations.containsKey(jobId)) {
//				kvStateLocations.put(jobId, new HashMap<>());
//			}
//
//			Map<String, KvStateLocation> queryNameLocationMap = kvStateLocations.get(jobId);
//			queryNameLocationMap.put(registrationName, location);
//
...
//			return CompletableFuture.completedFuture(new KvStateLocationResponse(serverAddress));
//		}
//
//		return CompletableFuture.completedFuture(new KvStateLocationResponse(null));
	...
	private void executeActionAsync(
		...
		if (!result.isDone()) {
			final CompletableFuture<KvStateLocationResponse> operationFuture = getState(request, update);
	...
	private CompletableFuture<KvStateLocationResponse> getState(
			final KvStateLocationRequest request,
			final boolean forceUpdate) {

		return getKvStateLookupInfo(request.getJobId(), request.getQueryName(), forceUpdate)
